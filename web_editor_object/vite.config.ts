...
function generateUnifiedDiff(beforeText: string, afterText: string, fileName: string): string {
  ...
  for (let i = aHunkStart; i < prefix; i++) out.push(` ${a[i]}`)
  for (let i = prefix; i < aChangeEnd; i++) out.push(`-${a[i]}`)
  for (let i = prefix; i < bChangeEnd; i++) out.push(`+${b[i]}`)
  for (let i = aChangeEnd; i < aHunkEnd; i++) out.push(` ${a[i]}`)
  return out.join('\n') + '\n'
}

function listPngsRecursive(rootDir: string, relPrefix = ''): Array<{ relPath: string }> {
  const output: Array<{ relPath: string }> = []
...
function safeResolveInside(baseDir: string, relativePath: string): string {
...
function objectEditorPlugin(): Plugin {
  return {
    ...
    configureServer(server) {
      ...
      server.middlewares.use('/__api/object-editor/preview-diff', async (req, res) => {
        ...
        try {
          const body = await readJsonBody<{ fileName: string; json: unknown }>(req)
      ...
      server.middlewares.use('/__api/object-editor/save', async (req, res) => {
        ...
        try {
          ...
          const imageWrites: Array<{ relPath: string; absPath: string; bytes: Buffer }> = []
