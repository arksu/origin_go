...
Требует у сущности компоненты Position и Perception (для игроков/наблюдателей).
...
Перед запуском — заполнить spatial hash (можно переиспользовать CollisionGrid).
Для каждого игрока с Perception:
