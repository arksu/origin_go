...
## Механизмы сглаживания начала движения

Реализованы два механизма для устранения рывков при начале движения:

### 1. Синтетический pre-roll keyframe
При получении первого пакета движения после остановки автоматически создаётся дополнительный keyframe:
- **Позиция**: текущая визуальная позиция игрока
- **Время**: -(1000/tickRate)мс от первого серверного keyframe (один тик сервера)
- **Velocity**: копируется из первого серверного пакета

Это обеспечивает плавную интерполяцию от текущей позиции к первому серверному положению. Offset динамически рассчитывается на основе `tickRate` из `S2C_PlayerEnterWorld`.

### 2. Адаптивная коррекция ошибок (ramp-in)
Скорость коррекции ошибок плавно увеличивается в первые 200мс движения:
- **Начальная скорость**: 0.03 (медленная коррекция)
- **Конечная скорость**: 0.15 (нормальная коррекция)
- **Функция сглаживания**: ease-in quadratic

Это предотвращает резкие скачки при начале движения.
