...
### Критерии завершения
- [ ] Клик по карте отправляет `MoveTo` с корректными координатами и modifiers.
- [ ] Клик по объекту отправляет `MoveToEntity` (объект имеет приоритет над землёй).
- [ ] Drag middle mouse перемещает камеру и **не** отправляет команду движения.
- [ ] Камера следует за игроком плавно (без jitter, источник — визуальная позиция).
- [ ] Масштабирование колесом работает, не скроллит страницу, zoom ограничен min/max.
- [ ] После `blur`/`visibilitychange` нет “залипших” состояний (drag/modifiers).
---
