...
from unfaker_content_adaptive import content_adaptive_downscale
from unfaker_wu_quantizer import WuQuantizer

# Import accelerated versions when available
try:
    from unfaker_pixel_rust_integration import (
        RUST_AVAILABLE,
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  unfaker.py input.png output.png              # Basic processing
  unfaker.py input.png output.png -c 16        # 16 colors, dominant downscaling
  unfaker.py input.png output.png --auto-colors
  unfaker.py input.png output.png --scale 4
  unfaker.py input.png output.png --cleanup morph,jaggy
        """,
    )
    ...
    parser.add_argument(
        ...
        nargs="?",
        default=None,
        help="Output image file (positional form for pipeline compatibility)",
    )
    parser.add_argument(
    ...
    try:
        output_path = args.output or args.output_positional or "output.png"
        ...
        if not args.quiet:
            ...
            print(f"  Output: {output_path} ({manifest.final_size[0]}x{manifest.final_size[1]})")
