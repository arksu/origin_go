...
- A new ordinary map click during an action-owned approach cancels that approach and its pending effect before ordinary click routing runs. While a target action is selecting, its map click takes precedence over dropping an item held in the hand, even when an object action lets an empty-ground click fall through to movement.
