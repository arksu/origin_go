...
func TestPlayerActionRoutesMapClickAndIgnoresRetiredActions(t *testing.T) {
	...
	for index, button := range []netproto.MapClickButton{netproto.MapClickButton_MAP_CLICK_BUTTON_PRIMARY, netproto.MapClickButton_MAP_CLICK_BUTTON_SECONDARY} {
		g.handlePlayerAction(client, uint32(index+2), &netproto.C2S_PlayerAction{Action: &netproto.C2S_PlayerAction_MapClick{MapClick: &netproto.MapClick{X: 42, Y: 99, TargetEntityId: 777, Button: button}}})
		...
		if !ok || click.X != 42 || click.Y != 99 || click.TargetEntityId != 777 || click.Button != button {
